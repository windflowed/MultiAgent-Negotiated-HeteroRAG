...
> **实验运行模型**：本节所有对比实验与评测均由 `deepseek-v4-flash`（DeepSeek API）驱动，包括路由分类、SQL 生成、文档三元组抽取、KG 实体链接、答案生成等所有 LLM 调用。
...
三组基线系统与主系统的对比实验（5类场景各1题，响应时延）：

| 系统 | sql_only | doc_only | kg_only | sql_doc | all_sources | 平均时延 |
|------|----------|----------|---------|---------|-------------|----------|
| 主系统（多Agent协商融合） | 69.8s | 38.2s | 11.6s | 52.2s | 36.8s | **41.7s** |
| 基线1：单模态RAG | 1.8s | 1.7s | 2.0s | 1.5s | 2.2s | 1.8s |
| 基线2：简单拼接多源RAG | 3.7s | 3.5s | 4.0s | 2.9s | 5.8s | 4.0s |
| 基线3：无协商多Agent RAG | 3.3s | 3.5s | 4.6s | 3.1s | 6.6s | 4.2s |

分析：

- **主系统优势**：动态路由避免无关数据源检索（如 kg_only 场景仅 11.6s）；协商融合保证答案完整且可溯源
- **单模态基线局限**：仅文档检索，数值类问题（sql_only）无法回答
- **拼接/无协商基线不足**：固定激活全数据源，无冲突消解，多源信息可能包含矛盾
- 主系统首次查询含资源初始化冷启动时间（BM25 索引加载约 35s），后续查询更快

### 嵌入模型选择

本系统采用 `BAAI/bge-small-zh-v1.5`（512维）作为中文向量化模型。与其他常见嵌入方案对比：

| 嵌入方案 | 维度 | 中文优化 | 本地部署 | 本系统选择 |
|----------|------|----------|----------|------------|
| BGE-small-zh-v1.5 | 512 | ✅ 专为中文优化 | ✅ CPU可运行 | ✅ 选用 |
| OpenAI text-embedding-3 | 1536 | 通用多语言 | ❌ 需API | 不适用（PRD本地约束） |
| paraphrase-multilingual-MiniLM | 384 | 通用 | ✅ CPU可运行 | 不适用（中文效果次于BGE） |

BGE-small-zh-v1.5 在中文语义检索场景下优于通用多语言模型，本地 CPU 即可运行，无需外部 API，完全符合项目本地部署约束。
